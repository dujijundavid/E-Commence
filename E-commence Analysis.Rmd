...
STEP 1: Environment set up
    ...
    c. use fread to load 4 data tables
    d. sanity check for data tables
```{r}
start_time=Sys.time()
# 1
library(data.table)
...
STEP 2: Get targeted audience 
  a. dorman_3month: select users who purchase once before 2016/12/22, then didn't purchase anything between 2016/12/22 and 2017/02/22, this group of users went dormant for 3 months.
...
#  customers who only made one purchase before 2016/12/22 
# method 1
tic()
base = subset( 
    order_table[
...
tic()
base2=sqldf("SELECT count(order_id) as count, max(order_amount) as order_amount, max(order_date) as order_date, product_id=max(product_id)
             FROM order_table 
             WHERE order_date<'20161222' & order_amount>0  
             GROUP BY customer_id 
              Having count==1")
toc()

# data.table data manipulation is 5.5 times more efficient compare to SQL
...
#data.table
  Notes: https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html
  #   R:    i   |  j                | by
  # SQL:  where | order by   select | update  group by
  
  Select column return vector:  ans <- flights[, arr_delay]
  Select column return data.table: ans <- flights[, list(arr_delay)]
...
#Data type:
as.numeric, as.character, data.matrix

#Feature engineering:

#Machine learning
  Modeling:
  Lasso: model_glm_cv_lasso = cv.glmnet(data.matrix(train_x),train_y_categorial,alpha =         1,family="binomial",type.measure="auc",parallel=TRUE)
  ...
  Predict
    1. lasso_predict = predict(model_glm_cv_lasso, data.matrix(test_x),type='response')
    2. lasso_pred = prediction(lasso_predict,test_y)
